...
${\partial log N(x \vert \mu,\sigma,\rho)}\over{\partial \hat{\mu}_1}$ = ![equation](https://latex.codecogs.com/svg.latex?=%20%20{{C}\over{\sigma_1}}%20\big{(}%20{{x_1%20-%20\mu_1}\over{\sigma_1}}%20-%20{{\rho(x_2%20-%20\mu_2)}\over{\sigma_2}}\big{)})

${{
  \partial log N(x \vert \mu,\sigma,\rho)
}\over{
  \partial \hat{\mu}_2
}}$ =![equation](https://latex.codecogs.com/svg.latex?{{C}\over{\sigma_1}}\big{(}{{x_2%20-%20\mu_2}\over{\sigma_2}}%20-%20{{\rho(x_1%20-%20\mu_1)}\over{\sigma_1}}\big{)})

${{
  \partial log N(x \vert \mu,\sigma,\rho)
}\over{
  \partial \hat{\sigma}_1
}}$ = ![equation](https://latex.codecogs.com/svg.latex?{{C(x_1%20-%20\mu_1)}\over{\sigma_1}}%20%20\big{(}%20{{x_1%20-%20\mu_1}\over{\sigma_1}}%20-%20{{\rho(x_2-\mu_2)}\over{\sigma_2}}%20\big{)})

${{
  \partial log N(x \vert \mu,\sigma,\rho)
}\over{
  \partial \hat{\sigma}_2
}}$ = ![equation](https://latex.codecogs.com/svg.latex?{{C(x_2%20-%20\mu_2)}\over{\sigma_2}}%20%20\big{(}%20{{x_2%20-%20\mu_2}\over{\sigma_2}}%20-%20{{\rho(x_1-\mu_1)}\over{\sigma_1}}%20\big{)})
